...
for ii = 1: numel(stimlist)
    ...
    if length(ftmcs) ~= length(ft)
        ...
        if (length(ftmcs) - length(ft)) < 0
            disp('correcting number of pulses');
            ...
            stimdat(ii).electricalstimulus.pulsetemplates = stimdat(ii).electricalstimulus.pulsetemplates(ftidx,:,:);
            ft = transpose(ft);
        else
            ft = ftmcs;
        end
