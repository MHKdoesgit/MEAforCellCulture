...
function [ops] = convertMsrdH5ToRawBinary(ops)
...
for imcd=1:numel(h5filenames)
    ...
    % there are two  analog stream, the second one is the filtered data
    stimdata{imcd}.Recording{1}.AnalogStream{2}.ChannelDataTimeStamps(end);
    stimsamples(imcd) = size(stimdata{imcd}.Recording{1}.AnalogStream{2}.ChannelDataTimeStamps,2);
    
end
H5fileInfo = stimdata{imcd}.Recording{1}.AnalogStream{2}.Info;
...
for iFile=1:Nfiles
    ...
    h5dat = stimdata{iFile}.Recording{1}.AnalogStream{2};
    ...
    for iChunk=1:Nchunk
        ...
        dat=int16(dat.ChannelData(chanMap,:));
