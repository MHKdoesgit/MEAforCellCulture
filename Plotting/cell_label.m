...
function [cell_titlename, png_filename] = cell_label(stimname, chinfo, experimentdate, iter, varargin)
%
%%% rgcname %%%
%
...
if isstruct(chinfo)
    if breakto2line
        cell_titlename ={[stimname,' Analysis for Channel ',num2str(chinfo.ch),', Cluster ',num2str(chinfo.clus),...
        ...
        png_filename = [cell_titlename{1},', quality ',num2str(chinfo.quality), ', expdate ',experimentdate];
    else
        cell_titlename =[stimname,' Analysis for Channel ',num2str(chinfo.ch),', Cluster ',num2str(chinfo.clus),...
            ', KS ID ',num2str(chinfo.id),', quality ',num2str(chinfo.quality),', nspk ',num2str(chinfo.n_spikes),...
            ', ',strrep(chinfo.comment{1},'_',' '),', for Experiment on ',experimentdate];
        png_filename = [extractBefore(cell_titlename,', nspk '),', expdate ',experimentdate];
    end
    
else
    
    if breakto2line
        cell_titlename ={[stimname,' Analysis for Channel ',num2str(chinfo(1,1)),', Cluster ',num2str(chinfo(1,2)),','];...
            [' for Experiment on ',experimentdate]};
        png_filename = [cell_titlename{1},cell_titlename{2}];
    else
        cell_titlename =[stimname,' Analysis for Channel ',num2str(chinfo(1,1)),', Cluster ',num2str(chinfo(1,2)),...
            ', for Experiment on ',experimentdate];
        png_filename = cell_titlename;
    end
    
end

cell_titlename = [num2str(iter,'%02g-'), cell_titlename];
png_filename = [num2str(iter,'%02g-'), png_filename];
